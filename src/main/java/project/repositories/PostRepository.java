...
public interface PostRepository extends CrudRepository<Post, Integer> {
     ...
     */
    List<Post> findAllByModerationStatusAndIsActiveOrderByTimeDesc(ModerationStatus moderationStatus, Byte active,
                                                                   Pageable pageable);

    /**
     *Запрос для api/post/early
     */
    List<Post> findAllByModerationStatusAndIsActiveOrderByTimeAsc(ModerationStatus moderationStatus, Byte active,
                                                                  Pageable pageable);
     ...
     */
    Post getByIdAndModerationStatusAndIsActive(Integer id, ModerationStatus moderationStatus, Byte active);
    ...
    @Query(value = "SELECT posts.* FROM posts " +
            ...
            "AND posts.moderation_status = 'ACCEPTED' " +
            "AND posts.time <= now() " +
            "AND tags.name = :tag_name", nativeQuery = true)
    ...
    List<Post> findAllByTitleContainingAndIsActiveAndModerationStatusAndTimeIsBeforeOrderByTimeDesc
            (String title, Byte isActive, ModerationStatus moderationStatus, LocalDateTime time, Pageable pageable);
    ...
    @Query(value = "SELECT * FROM posts " +
            ...
            "AND posts.is_active = 1 " +
            "AND posts.moderation_status = :moderation_status", nativeQuery = true)
    List<Post> getAllMyPosts(@Param("user_id") Integer userId, @Param("moderation_status") String moderationStatus,
                             Pageable pageable);

    /**
     * Получение количества активных постов определенного пользователя с определенными статусами модерации
     */
    @Query(value = "SELECT count(*) FROM posts " +
            ...
            "AND moderation_status = :moderation_status " +
            "AND time <= now()", nativeQuery = true)
    Integer countAllMyPosts(@Param("user_id") Integer userId, @Param("moderation_status") String moderationStatus);
    ...
    @Query(value = "SELECT * FROM posts " +
            "WHERE posts.user_id = :user_id " +
            "AND posts.is_active = 0", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT time FROM posts " +
            "WHERE is_active = 1 " +
            "AND moderation_status = 'ACCEPTED' " +
            "AND time <= now() " +
    ...
    @Query(value = "SELECT count(*) FROM post_votes " +
            ...
            "AND posts.moderation_status = 'ACCEPTED' " +
            "AND posts.time <= now() " +
            "AND post_votes.value = :value", nativeQuery = true)
     ...
     */
    List<Post> findAllByIsActiveAndModerationStatusAndAndModeratorId(Byte active, ModerationStatus moderationStatus,
                                                                     Integer moderatorId, Pageable pageable);

    /**
     * поиск количества постов, которые утвердил или отклонил определенный модератор
     */
    Integer countAllByModerationStatusAndAndIsActiveAndAndModeratorId(ModerationStatus moderationStatus, Byte active,
                                                                      Integer moderatorId);
    ...
    /**
     * поиск айди юзера по айди поста, используется для проверки при увеличении количества просмотров поста
     */
    @Query(value = "SELECT user_id FROM posts " +
